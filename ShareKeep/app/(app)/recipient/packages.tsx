...
function PackageCard({ pkg }: { pkg: Package }) {
  ...
  const handlePress = () => {
    const params = { packageName: pkg.name, trackingNumber: pkg.trackingNumber };
    if (pkg.status === 'waiting') {
      router.push({ pathname: '/(app)/recipient/matching', params });
    } else if (pkg.status === 'stored') {
      router.push({ pathname: '/(app)/recipient/pickup-ready', params });
    }
    // TODO: completed → 完了画面へ遷移
  };
