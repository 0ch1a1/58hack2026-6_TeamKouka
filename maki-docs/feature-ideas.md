...
主役は **案2 スコアリングマッチング ＋ 案5 監査ログ／CO2可視化 ＋ 演出系**。署名QR（案1）は「現場でも止まらない」補強トピックに降格。
...
5. **案7 LLMトラブル一次対応** — Edge Function経由
6. **案8 ライブ位置トラッキング**（ダミー座標＋Realtime）
7. **案1 署名付き短期有効QR** — スコープ管理必須。ハッカソン本番は「オンラインQR検証まで」に削り、署名・オフラインは余力枠
8. **案3 AI伝票照合** — 照合のみに限定
...
## 地域全体の ShareKeep マップ（集計カード）

- 概要：近所単位の CO2削減合計・再配達防止回数・協力者数。地図でなく集計カードで軽量に
- クエリ例：
  ```sql
